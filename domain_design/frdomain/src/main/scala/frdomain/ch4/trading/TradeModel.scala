...
trait TradeModel {this: RefModel =>
  ...
  case class Trade private[trading] (account: Account, instrument: Instrument, refNo: String, 
    market: Market, unitPrice: BigDecimal, quantity: BigDecimal, tradeDate: Date = today,
  ...
  // tax and fees applicable for each market; Other signifies the general rule
  val taxFeeForMarket: Map[Market, List[TaxFeeId]] = 
    Map(Other -> List(TradeTax, Commission), Singapore -> List(TradeTax, Commission, VAT))

  // get the list of tax/fees applicable for this trade depends on the market
  ...
  def makeTrade(account: Account, instrument: Instrument, refNo: String, market: Market, 
    unitPrice: BigDecimal, quantity: BigDecimal, td: Date = today, vd: Option[Date] = None): Trade = {
