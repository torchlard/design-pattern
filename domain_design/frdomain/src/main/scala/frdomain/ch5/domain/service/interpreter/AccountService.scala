...
class AccountServiceInterpreter extends AccountService[Account, Amount, Balance] {
  ...
  def open(no: String, name: String, rate: Option[BigDecimal], openingDate: Option[Date],
    ...
    repo.query(no) match {
      // \/-(_): match right(in this case Account) ; -\/(_): match left  | right-biased Either
      ...
      case a @ -\/(_) => a    // match NonEmptyList[String]: error msg?
    }
  }
  ...
  def balance(no: String) = kleisli[Valid, AccountRepository, Balance] { 
