...
object Main {
  // setup Akka system and materializer
  ...
  val accountNos: Source[String, akka.NotUsed] =  // NotUsed: not important, Unit
  ...
  val netTxn: RunnableGraph[Future[akka.Done]] = 
    transactions.map(validate)
                ...
                .fold(TransactionMonoid.zero)(_ |+| _)  // fold over substream
                // .async
                .mergeSubstreams
  ...
  val graph = RunnableGraph.fromGraph(GraphDSL.create(netTxnSink) { implicit b => ms =>
    ...
    // construct graph as a domain pipeline: end with 2 sinks [net transaction + audit]
